...
def load_best_creature(filename="best_creature.save"):
    ...
    try:
        ...
        nn = NeuralNetwork(save_data['params']['NUM_INPUTS'], 10, save_data['params']['NUM_OUTPUTS'])
...
class Creature:
    ...
    NUM_INPUTS = 5  # or whatever the correct number of inputs is
    NUM_OUTPUTS = 4 
    ...
    def update(self, apples, score_zone,enemy_pos, dt=1.0):
        ...
        if enemy_pos is not None:
            ...
            if dist_to_enemy < 25:
                # Flee the enemy by turning and accelerating in the opposite direction
                direction_to_enemy = np.arctan2(enemy_pos[1] - self.pos[1], enemy_pos[0] - self.pos[0])
                self.angle = direction_to_enemy + math.pi  # Move in the opposite direction
                accel_request = 1.0  # Speed up to escape
        ...
        if self.energy > 0:
            ...
            # Replace the boundary check in update() with this:
...
def selection(population):
    # Tournament selection instead of just top half
    parents = []
    for _ in range(len(population) // 2):
        # Pick 4 random creatures, take the best 2
...
def reproduction(parents, generation):
    ...
    # Reproduce new creatures by crossover and mutation
    while len(next_population) < POPULATION_SIZE:
        parent1 = random.choice(parents)
        parent2 = random.choice(parents)
        child_nn = NeuralNetwork(parent1.nn.NUM_INPUTS, 10, parent1.nn.NUM_OUTPUTS)
        child_nn.set_weights(crossover(parent1.nn.get_weights(), parent2.nn.get_weights()))
        child_nn.mutate(MUTATION_RATE, MUTATION_STRENGTH)
        next_population.append(Creature(random.randint(0, SCREEN_WIDTH), random.randint(0, SCREEN_HEIGHT), child_nn))

    return next_population

def crossover(weights1, weights2):
    # Simple crossover: take half from each parent
    mid_point = len(weights1) // 2
    return np.concatenate([weights1[:mid_point], weights2[mid_point:]])
# --- Apple Class ---
class Apple:
...
def reproduction(parents, generation):
    ...
    while len(next_population) < POPULATION_SIZE:
        ...
        # More complex crossover (e.g., single-point) could be used
